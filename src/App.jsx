...
function Sidebar({ active, setActive, onLogout, mobile, onClose, theme, onTheme }) {
  return (
    <div className={cn('flex flex-col h-full bg-[#070d1e] border-r border-slate-800/80', mobile ? 'w-72 p-5' : 'w-64 p-6')}>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/25">
            <Monitor className="w-5 h-5 text-white"/>
          </div>
          <span className="font-bold text-lg tracking-tight">Solar<span className="text-blue-500">Axis</span></span>
        </div>
        {mobile && <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-800 text-slate-500"><X className="w-4 h-4"/></button>}
      </div>

      <nav className="flex-1 space-y-1">
        {NAV.map(n => (
          <button key={n.id} onClick={() => { setActive(n.id); if (mobile) onClose(); }}
            className={cn('w-full flex items-center justify-between px-3.5 py-3 rounded-2xl transition-all text-sm group',
              active===n.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/25' : 'text-slate-500 hover:bg-slate-800/50 hover:text-slate-200')}>
            <div className="flex items-center gap-3">
              <n.icon className={cn('w-[18px] h-[18px]', active===n.id ? 'text-white' : 'text-slate-600 group-hover:text-blue-400 transition-colors')}/>
      ...
      {/* Theme Quick Pick — tiny dots at bottom of nav */}
      ...
      <div className="pt-5 border-t border-slate-800 mt-4">
        <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-800/20 border border-slate-800/50">
          <div className="w-9 h-9 rounded-full bg-blue-500/20 border border-blue-500/20 flex items-center justify-center text-blue-300 font-bold text-sm flex-shrink-0">V</div>
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-semibold text-slate-200 truncate">Vendor Admin</p>
            <p className="text-[11px] text-slate-500 truncate">solar.vendor@api.com</p>
          </div>
...
function TopBar({ onMenuOpen, onBell, onSearch }) {
  return (
    <div className="lg:hidden flex items-center justify-between px-4 py-3.5 border-b border-slate-800 bg-[#070d1e]/90 backdrop-blur sticky top-0 z-30">
      <button onClick={onMenuOpen} className="p-2 rounded-xl hover:bg-slate-800 text-slate-400"><Menu className="w-5 h-5"/></button>
      <span className="font-bold text-sm text-slate-200">Solar<span className="text-blue-500">Axis</span></span>
      <div className="flex items-center gap-1">
        <button onClick={onSearch} className="p-2 rounded-xl hover:bg-slate-800 text-slate-400"><Search className="w-4 h-4"/></button>
        <button onClick={onBell} className="p-2 rounded-xl hover:bg-slate-800 text-slate-400 relative">
...
function BottomNav({ active, setActive }) {
  return (
    <div className="lg:hidden fixed bottom-0 inset-x-0 bg-[#070d1e]/95 backdrop-blur-xl border-t border-slate-800 z-30">
      <div className="flex items-center justify-around px-2 py-2">
        {NAV.slice(0,5).map(n => (
          <button key={n.id} onClick={() => setActive(n.id)} className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-2xl relative">
            <n.icon className={cn('w-5 h-5 transition-colors', active===n.id ? 'text-blue-400' : 'text-slate-600')}/>
            <span className={cn('text-[9px] font-semibold', active===n.id ? 'text-blue-400' : 'text-slate-600')}>{n.label.split(' ')[0]}</span>
            {n.badge > 0 && <span className="absolute top-0.5 right-2 w-4 h-4 rounded-full bg-red-500 text-white text-[8px] font-bold flex items-center justify-center">{n.badge}</span>}
          </button>
        ))}
      </div>
    </div>
...
function DesktopHeader({ tab, onSearch, onBell, bellCount, onTheme, onShortcuts, isFullscreen, onFullscreen }) {
  return (
    <header className="hidden lg:flex items-center justify-between mb-8 flex-shrink-0">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-slate-100">{PAGE_TITLES[tab]}</h1>
        <p className="text-xs text-slate-500 mt-0.5">SolarAxis Monitor · Vendor Portal</p>
      </div>
      <div className="flex items-center gap-2">
        {/* Search / Command Palette trigger */}
        <button onClick={onSearch}
          className="flex items-center gap-2 bg-slate-900/50 border border-slate-800 hover:border-slate-700 rounded-full py-2 pl-3 pr-4 text-sm text-slate-500 hover:text-slate-300 transition-all w-48 group">
        ...
        {/* Bell */}
        <button onClick={onBell} className="p-2.5 rounded-full bg-slate-900/50 border border-slate-800 hover:border-slate-700 text-slate-500 hover:text-blue-400 relative transition-colors">
          <Bell className="w-4 h-4"/>
          {bellCount > 0 && <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full bg-red-500"/>}
        </button>

        {/* Theme */}
        <button onClick={onTheme} className="p-2.5 rounded-full bg-slate-900/50 border border-slate-800 hover:border-slate-700 text-slate-500 hover:text-purple-400 transition-colors" title="Themes (T)">
          <Palette className="w-4 h-4"/>
        </button>

        {/* Keyboard shortcuts */}
        <button onClick={onShortcuts} className="p-2.5 rounded-full bg-slate-900/50 border border-slate-800 hover:border-slate-700 text-slate-500 hover:text-slate-300 transition-colors" title="Shortcuts (?)">
          <Keyboard className="w-4 h-4"/>
        </button>

        {/* Fullscreen */}
        <button onClick={onFullscreen} className="p-2.5 rounded-full bg-slate-900/50 border border-slate-800 hover:border-slate-700 text-slate-500 hover:text-slate-300 transition-colors" title="Fullscreen (F)">
...
function LiveTicker() {
  ...
  return (
    <div className="bg-blue-600/8 border-y border-blue-500/10 py-1.5 overflow-hidden relative">
      <div className="flex gap-10 animate-ticker whitespace-nowrap">
        {[...items, ...items].map((item, i) => (
          <span key={i} className="text-[11px] font-semibold text-blue-400/70 flex items-center gap-2">
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    if (email !== creds.email || pwd !== creds.pwd) {
      setError('Incorrect email or password. Check the demo credentials below.');
      return;
    }
    setError('');
    setLoading(true);
  ...
  return (
    <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <motion.div initial={{opacity:0,scale:0.95,y:16}} animate={{opacity:1,scale:1,y:0}}
        className="w-full max-w-sm bg-[#0b1628]/90 backdrop-blur-2xl border border-white/8 p-8 rounded-[2.5rem] shadow-2xl relative z-10">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex items-center gap-0 bg-[#0f172a] border border-[#1e293b] rounded-2xl overflow-hidden focus-within:border-blue-500/50 focus-within:shadow-[0_0_0_2px_rgba(59,130,246,0.15)] transition-all">
...
export default function App() {
  ...
  if (role === 'customer') return <CustomerPortal onLogout={() => setRole(null)} />;
  return <MainApp onLogout={() => setRole(null)} />;
}
